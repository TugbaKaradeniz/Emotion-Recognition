...
def detect_faces(emotion):
    files = glob.glob("CK+/sorted_set//%s//*" %emotion) #Get list of all images with emotion

    filenumber = 0
    for f in files:
        gray = cv2.imread(f, 0) #Open image
        ...
        face2 = faceDet2.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10, 
                                          minSize=(5, 5), flags=cv2.CASCADE_SCALE_IMAGE)
        face3 = faceDet3.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10, 
                                          minSize=(5, 5), flags=cv2.CASCADE_SCALE_IMAGE)
        face4 = faceDet4.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10, 
                                          minSize=(5, 5), flags=cv2.CASCADE_SCALE_IMAGE)
        ...
        for (x, y, w, h) in facefeatures: #get coordinates and size of rectangle containing face
            ...
            try:
                ...
                cv2.imwrite("CK+/dataset//%s//%s.jpg" %(emotion, filenumber), out) #Write image
            except:
